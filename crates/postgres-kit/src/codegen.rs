...
pub struct CodegenOptions {
    ...
    /// Full override of the leading comment header (everything before the
    /// optional `REDACTED` line and `#![allow(dead_code)]`). When `Some`, this
    /// text is emitted verbatim (a trailing newline is added if missing) in place
    /// of the default `// @generated by postgres-kit codegen …` block, and
    /// `provenance` is ignored. This exists so a consuming generator can emit a
    /// header byte-identical to an existing hand-rolled generator's — e.g. the
    /// smooai `gen-rust-rows.mts` header that embeds the Drizzle import path and
    /// table export name (which a `PgTableSpec` alone can't know) — making a
    /// drop-in cutover a no-op diff. See `CodegenOptions::header`.
    pub header: Option<String>,
    /// Override the table name shown in the row struct's doc comment
    /// (`/// sqlx FromRow row mirroring \`<name>\``). Defaults to `spec.name` (the
    /// DB table name). Set this to a source-schema identifier that differs from
    /// the DB name — e.g. the smooai Drizzle export name (`slackIntegrations` for
    /// table `integration_slack`) — so the generated doc matches a bridged
    /// generator byte-for-byte. Cosmetic only (doc comment), never affects SQL.
...
impl CodegenOptions {
    ...
    /// Fully override the leading comment header (see [`CodegenOptions::header`]).
    /// Takes precedence over `provenance`.
    ...
    /// Override the table name shown in the row struct's doc comment
    /// (see [`CodegenOptions::source_name`]).
...
pub fn emit_rust_module(spec: &PgTableSpec, opts: &CodegenOptions) -> Result<String, CodegenError> {
    ...
    if let Some(header) = &opts.header {
        // Verbatim header override (e.g. to match an existing generator's bytes).
        out.push_str(header);
        if !header.ends_with('\n') {
            out.push('\n');
        }
    ...
    let doc_name = opts.source_name.as_deref().unwrap_or(&spec.name);
    out.push_str(&format!(
        "/// sqlx FromRow row mirroring `{doc_name}`, camelCase JSON.\n"
    ));
    out.push_str(
...
";
    ...
    /// Byte-for-byte reproduction of a real committed smooai row module
    /// (`rust/api-prime/src/generated/security_target_row.rs`, generated today by
    /// `gen-rust-rows.mts`) from a `PgTableSpec` + a verbatim header override.
    /// This proves the kit's codegen can drop in for the bespoke TS generator for
    /// a representative org-scoped table with an enum column (→ `::text` cast),
    /// nullable columns (→ `Option`), camelCase renames, the `COLUMNS` const, and
    /// the org-scoped marker + `impl` — given the columns in declaration order.
    /// (The enum/numeric `::text` rule and the org marker are the load-bearing
    /// parts; the header override is what closes the last byte-equivalence gap,
    /// since the `.mts` header embeds the Drizzle import path + export name that a
    /// `PgTableSpec` alone cannot know.)
    #[test]
    fn golden_reproduces_committed_smooai_row_via_header_override() {
        let spec = PgTableSpec::new(
            ...
            vec![
                ...
                col("organization_id", PgType::Uuid),
                col("url", PgType::Text),
                col("label", PgType::Text).nullable(),
                col(
                    "auth_kind",
                    PgType::Enum("security_target_auth_kind".into()),
                ),
                col("auth_secret_ref", PgType::Text).nullable(),
                col("enabled", PgType::Bool),
                col("last_scan_at", PgType::Timestamptz).nullable(),
                col("created_at", PgType::Timestamptz),
                col("updated_at", PgType::Timestamptz),
            ],
        )
        .primary_key(["id"]);

        let header = "\
// @generated by gen-rust-rows.mts from the Drizzle schema (@smooai/db/schemas/security/security-targets :: securityTargets).
// Do not edit by hand — run `pnpm gen:rust-rows`. Enum/numeric columns are cast to
// text in COLUMNS so sqlx decodes them into String (SMOODEV-1919, Code-Conventions #27).";

        let opts = CodegenOptions::new()
            .type_name("SecurityTargetRow")
            .source_name("securityTargets")
            .header(header);

        // The exact bytes of the committed file (it fits on one line, so rustfmt is
        // a no-op — the consuming generator still runs rustfmt for tables whose
        // COLUMNS const wraps past 100 cols).
        let expected = r##"// @generated by gen-rust-rows.mts from the Drizzle schema (@smooai/db/schemas/security/security-targets :: securityTargets).
// Do not edit by hand — run `pnpm gen:rust-rows`. Enum/numeric columns are cast to
// text in COLUMNS so sqlx decodes them into String (SMOODEV-1919, Code-Conventions #27).
#![allow(dead_code)]

/// sqlx FromRow row mirroring `securityTargets`, camelCase JSON.
#[derive(Debug, Clone, ::serde::Serialize, ::serde::Deserialize, ::sqlx::FromRow)]
pub struct SecurityTargetRow {
    pub id: ::uuid::Uuid,
    #[serde(rename = "organizationId")]
    pub organization_id: ::uuid::Uuid,
...
/// SELECT column list for `SecurityTargetRow` (enum/numeric columns cast to text).
pub const SECURITY_TARGET_ROW_COLUMNS: &str = r#"id, organization_id, url, label, auth_kind::text AS auth_kind, auth_secret_ref, enabled, last_scan_at, created_at, updated_at"#;

/// Org-scoped CRUD marker — `list_by_org` / `find_by_id` / `delete_by_id` bind
/// `organization_id` themselves (see [`crate::orm::OrgScopedTable`]).
pub struct SecurityTargets;
impl crate::orm::OrgScopedTable for SecurityTargets {
    type Row = SecurityTargetRow;
    const NAME: &'static str = "security_targets";
    const COLUMNS: &'static str = SECURITY_TARGET_ROW_COLUMNS;
}
"##;
        assert_eq!(emit_rust_module(&spec, &opts).unwrap(), expected);
    }

    #[test]
    fn header_override_adds_trailing_newline_and_ignores_provenance() {
        let spec = PgTableSpec::new("t", vec![col("id", PgType::Uuid)]);
        let opts = CodegenOptions::new()
            .provenance("ignored")
            .header("// custom header line"); // no trailing newline
        let out = emit_rust_module(&spec, &opts).unwrap();
        assert!(out.starts_with("// custom header line\n#![allow(dead_code)]\n"));
        assert!(!out.contains("ignored"));
        assert!(!out.contains("@generated by postgres-kit"));
    }
